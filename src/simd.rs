...
#[inline(always)]
unsafe fn load8(ptr: *const u8) -> u64 { unsafe {
    let mut w = 0u64;
...
pub static FORBIDDEN_HOST_TABLE: [bool; 256] = {
    let mut t = [false; 256];
    let bytes: &[u8] = &[0,9,10,13,32,35,47,58,60,62,63,64,91,92,93,94,124];
    let mut i = 0;
    while i < bytes.len() { t[bytes[i] as usize] = true; i += 1; }
    t
};
...
pub static FORBIDDEN_DOMAIN_TABLE: [bool; 256] = {
    let mut t = [false; 256];
    let bytes: &[u8] = &[0,9,10,13,32,35,47,58,60,62,63,64,91,92,93,94,124,37];
    let mut i = 0;
    while i < bytes.len() { t[bytes[i] as usize] = true; i += 1; }
    let mut c = 0usize; while c <= 32 { t[c] = true; c += 1; }
    let mut c = 127usize; while c < 256 { t[c] = true; c += 1; }
    t
};
...
fn swar_find4(b: &[u8], d0: u8, d1: u8, d2: u8, d3: u8) -> usize {
    let m0=broadcast(d0); let m1=broadcast(d1); let m2=broadcast(d2); let m3=broadcast(d3);
    let n = b.len();
    let mut i = 0;
    while i + 8 <= n {
        let w = unsafe { load8(b.as_ptr().add(i)) };
        if has_zero_byte(w^m0)|has_zero_byte(w^m1)|has_zero_byte(w^m2)|has_zero_byte(w^m3) != 0 {
            for j in 0..8 { if b[i+j]==d0||b[i+j]==d1||b[i+j]==d2||b[i+j]==d3 { return i+j; } }
        }
        i += 8;
    }
    for (k, &c) in b[i..].iter().enumerate() { if c==d0||c==d1||c==d2||c==d3 { return i+k; } }
    n
}

#[inline]
fn swar_find3(b: &[u8], d0: u8, d1: u8, d2: u8) -> usize {
    let m0=broadcast(d0); let m1=broadcast(d1); let m2=broadcast(d2);
    let n = b.len();
    let mut i = 0;
    while i + 8 <= n {
        let w = unsafe { load8(b.as_ptr().add(i)) };
        if has_zero_byte(w^m0)|has_zero_byte(w^m1)|has_zero_byte(w^m2) != 0 {
            for j in 0..8 { if b[i+j]==d0||b[i+j]==d1||b[i+j]==d2 { return i+j; } }
        }
        i += 8;
    }
    for (k, &c) in b[i..].iter().enumerate() { if c==d0||c==d1||c==d2 { return i+k; } }
    n
}
...
fn swar_find5(b: &[u8], d0:u8, d1:u8, d2:u8, d3:u8, d4:u8) -> usize {
    let m0=broadcast(d0); let m1=broadcast(d1); let m2=broadcast(d2);
    let m3=broadcast(d3); let m4=broadcast(d4);
    let n = b.len();
    let mut i = 0;
    while i + 8 <= n {
        let w = unsafe { load8(b.as_ptr().add(i)) };
        let h = has_zero_byte(w^m0)|has_zero_byte(w^m1)|has_zero_byte(w^m2)
               |has_zero_byte(w^m3)|has_zero_byte(w^m4);
        if h != 0 {
            for j in 0..8 {
                if b[i+j]==d0||b[i+j]==d1||b[i+j]==d2||b[i+j]==d3||b[i+j]==d4 { return i+j; }
            }
        }
        i += 8;
    }
    for (k, &c) in b[i..].iter().enumerate() {
        if c==d0||c==d1||c==d2||c==d3||c==d4 { return i+k; }
    }
    n
}
...
fn swar_has_tabs_or_newline(b: &[u8]) -> bool {
    let cr=broadcast(b'\r'); let lf=broadcast(b'\n'); let ht=broadcast(b'\t');
    let n = b.len();
    ...
    if i < n {
        let mut w: u64 = 0;
        unsafe { core::ptr::copy_nonoverlapping(b.as_ptr().add(i), &mut w as *mut u64 as *mut u8, n-i) };
        running |= has_zero_byte(w^cr)|has_zero_byte(w^lf)|has_zero_byte(w^ht);
    }
    running != 0
}
...
fn swar_classify(b: &[u8], low: &[u8; 16], high: &[u8; 16]) -> usize {
    for (i, &c) in b.iter().enumerate() {
        if low[(c & 0xF) as usize] & high[(c >> 4) as usize] != 0 { return i; }
    }
    b.len()
}
...
mod x86 {
    ...
    pub(super) unsafe fn find_delim_ssse3(
        b: &[u8],
        low: &[u8; 16],
        high: &[u8; 16],
    ) -> usize { unsafe {
        ...
        for (k, &c) in b[i..].iter().enumerate() {
            if low[(c & 0xF) as usize] & high[(c >> 4) as usize] != 0 { return i+k; }
        }
        n
    }}
    ...
    #[target_feature(enable = "ssse3")]
    pub(super) unsafe fn has_tabs_or_newline_ssse3(b: &[u8]) -> bool { unsafe {
        use core::arch::x86_64::*;
        ...
        if _mm_movemask_epi8(running) != 0 { return true; }
        b[i..].iter().any(|&c| c == b'\t' || c == b'\n' || c == b'\r')
    }}
}
...
mod neon {
    ...
    pub(super) unsafe fn find_delim_neon(
        b: &[u8],
        low: &[u8; 16],
        high: &[u8; 16],
    ) -> usize {
        ...
        while i + 16 <= n {
            ...
            if vmaxvq_u8(cl) != 0 {
                ...
                for (k, &v) in tmp.iter().enumerate() { if v != 0 { return i + k; } }
            }
            i += 16;
        }
        if i < n {
    ...
    pub(super) unsafe fn has_tabs_or_newline_neon(b: &[u8]) -> bool {
        ...
        if vmaxvq_u8(running) != 0 { return true; }
        b[i..].iter().any(|&c| c == b'\t' || c == b'\n' || c == b'\r')
    }
}
